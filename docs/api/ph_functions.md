...
|---|---|---|---|
| PHSEN | Sunburst SAMI-II pH | Moored (fixed depth) | pH Sensor |
| PHSEN-G | Sea-Bird Scientific Deep SeapHOx V2 | Moored (fixed depth) | pH Sensor, shallow-rated |
| PHSEN-H | Sea-Bird Scientific Deep SeapHOx V2 | Moored (fixed depth) | pH Sensor, deep-rated |

`ph_functions.py` processes data from the Sunburst SAMI-II pH (PHSEN) and
computes the L2 pH of seawater data product (PHWATER_L2). `phsen_h_functions.py`
processes data from the Sea-Bird Scientific Deep SeapHOx V2 (PHSEN-G and
PHSEN-H) and computes the same PHWATER_L2 data product. Both instruments
measure pH on the total hydrogen ion scale ($\text{pH}_T$). All calibration
coefficients are from factory calibration sheets supplied with individual
instruments.

---

### PHWATER — pH of Seawater (L0 to L2)

PHWATER is the OOI Level 2 pH of seawater on the total hydrogen ion scale
($\text{pH}_T$). Both PHSEN and PHSEN-G/H produce PHWATER_L2 directly from
their raw L0 data; there is no intermediate L1 product.

#### PHSEN — Sunburst SAMI-II pH

The SAMI-II pH measures pH using a colorimetric reaction with the pH indicator
...
to compute pH on the total scale (DPS 1341-00510, §3.1 and §3.2).
...
Raw signal intensities range from 0 to 4096 counts and correspond to a DC
voltage range of 0 to 5 V (DPS 1341-00510, §4.2).

**Thermistor Temperature**

The raw thermistor count is converted to temperature in degrees C. The
...
Beer's Law (DPS 1341-00510, §3.2, Equation 3):
...
24.788 $^\circ\text{C}$ (DPS 1341-00510, §4.2):

$$E_{a434} = e_{a434} - 26\,(T_C - 24.788)$$

$$E_{a578} = e_{a578} + (T_C - 24.788)$$

$$E_{b434} = e_{b434} + 12\,(T_C - 24.788)$$

$$E_{b578} = e_{b578} - 71\,(T_C - 24.788)$$

The absorptivity ratios used in the pH equation are:

$$e_1 = E_{a578} / E_{a434}, \quad e_2 = E_{b578} / E_{a434}, \quad e_3 = E_{b434} / E_{a434}$$

**pKa**

The apparent dissociation constant of mCP is computed from temperature and
salinity (Clayton and Byrne, 1993; DPS 1341-00510, §4.3):

$$pK'_a = \frac{1245.69}{T_C + 273.15} + 3.8275 + 0.0021\,(35 - S)$$

where $S$ is practical salinity (default 35.0 if no CTD data are available).

**PHWATER_L2 Calculation**

Point-by-point pH values are computed for each of the 23 sample measurement
sets (DPS 1341-00510, §3.2, Equation 2):

$$\text{pH}_\text{point} = pK'_a + \log_{10}\!\left(\frac{R - e_1}{e_2 - R\,e_3}\right)$$

Indicator concentrations for the protonated and deprotonated forms are:

$$[\text{HI}^-] = \frac{A_{434}\,E_{b578} - A_{578}\,E_{b434}}{E_{a434}\,E_{b578} - E_{b434}\,E_{a578}}$$

$$[\text{I}^{2-}] = \frac{A_{578}\,E_{a434} - A_{434}\,E_{a578}}{E_{a434}\,E_{b578} - E_{b434}\,E_{a578}}$$
...
indicator concentration (DPS 1341-00510, Appendix A).

An impurity correction is applied when the extrapolated pH exceeds 8.2:

$$\text{pH}_\text{final} = \text{pH} \times \text{ind\_slp} + \text{ind\_off}$$
...
$^\circ\text{C}$ and at salinities of $35 \pm 1$; salinity corrections
from a co-located CTD extend the valid salinity range (DPS 1341-00510,
§3.3).

---

#### PHSEN-G and PHSEN-H — Sea-Bird Scientific Deep SeapHOx V2
...
and is sensitive to chloride activity. The raw ISFET voltage is digitised
...
$$S_\text{nernst} = \frac{R\,T\,\ln(10)}{F}$$

where $R = 8.3144621\ \text{J}\,(\text{mol}\,\text{K})^{-1}$,
$T$ is temperature in K, and $F = 96485.365\ \text{C}\,\text{mol}^{-1}$.

The pressure response of the sensor is modelled by a 6th-order polynomial
...
$$\text{pH}_T = \frac{V_\text{FET/REF} - k_0 - k_2\,t - f(P)}{S_\text{nernst}}
+ \log_{10}(Cl_T) + 2\,\log_{10}(\gamma_{\pm\text{HCl}})_{T\&P}
- \log_{10}\!\left(1 + \frac{S_T}{K_{S,T\&P}}\right)
- \log_{10}\!\left(\frac{1000 - 1.005\,S}{1000}\right)$$

where $t$ is temperature in $^\circ\text{C}$, $S$ is practical salinity,
...
$$Cl_T = \frac{0.99889}{35.453} \times \frac{S}{1.80655} \times \frac{1000}{1000 - 1.005\,S}$$

**Sample ionic strength** (Dickson et al. 2007):

$$I = \frac{19.924\,S}{1000 - 1.005\,S}$$

**Debye-Hückel constant** (Khoo et al. 1977):

$$A_{DH} = 3.4286 \times 10^{-6}\,t^2 + 6.7503 \times 10^{-4}\,t + 0.49172143$$

**HCl activity coefficient — temperature only** (Khoo et al. 1977):

$$\log(\gamma_{\pm\text{HCl}})_T = \frac{-A_{DH}\,\sqrt{I}}{1 + 1.394\,\sqrt{I}} + (0.08885 - 0.000111\,t)\,I$$

**Partial molal volume of HCl** (Millero 1983):

$$\bar{V}_\text{HCl} = 17.85 + 0.1044\,t - 0.0001316\,t^2$$

**HCl activity coefficient — temperature and pressure** (Johnson et al. 2017):

$$\log(\gamma_{\pm\text{HCl}})_{T\&P} = \log(\gamma_{\pm\text{HCl}})_T + \frac{\bar{V}_\text{HCl}\,p}{2\,\ln(10)\,R\,T\,10}$$
...
**Acid dissociation constant of $\text{HSO}_4^-$ at $T$** (Dickson et al. 2007):

$$K_S = (1 - 0.001005\,S)\,\exp\!\left(\frac{-4276.1}{T} + 141.328 - 23.093\ln T\right.$$
$$\left. + \left(\frac{-13856}{T} + 324.57 - 47.986\ln T\right)\sqrt{I} + \left(\frac{35474}{T} - 771.54 + 114.723\ln T\right)I - \frac{2698}{T}\,I^{1.5} + \frac{1776}{T}\,I^2\right)$$

**Partial molal volume of $\text{HSO}_4^-$** (Millero 1983):

$$\bar{V}_S = -18.03 + 0.0466\,t + 0.000316\,t^2$$

**Compressibility of $\text{HSO}_4^-$** (Millero 1983):

$$\bar{K}_S = \frac{-4.53 + 0.09\,t}{1000}$$

**Acid dissociation constant corrected for $T$ and $P$** (Millero 1982):

$$K_{S,T\&P} = K_S\,\exp\!\left(\frac{-\bar{V}_S\,p + 0.5\,\bar{K}_S\,p^2}{R\,T\,10}\right)$$
