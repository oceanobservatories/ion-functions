...
"""
@package ion_functions.qartod_functions
@file ion_functions/qartod_functions.py
@author Seth Foote
@brief Copy of QARTOD QC functions from ioos qartod repository:
https://github.com/ioos/qartod/blob/master/ioos_qartod/qc_tests/qc.py#L83
"""
import numpy as np
import pyproj
...
# state.  This is a bit hardcoded. If needed, one can change the id
# attribute to support different internal representations of QARTOD tests.
...
def set_prev_qc(flag_arr, prev_qc):
    """Takes previous QC flags and applies them to the start of the array
       ...
       :param prev_qc: An array of previous QC values corresponding to the start
                       position of flag_arr
       """
    cond = prev_qc != QCFlags.NOT_EVALUATED
    flag_arr[cond] = prev_qc[cond]


@add_qartod_ident(3, 'Location Test')
def location_set_check(lat, lon, bbox_arr=([-180, -90], [180, 90]),
                       range_max=None):
    """
    ...
    :param lat: Latitude expressed as a floating point value (degrees)
    :param lon: Longitude expressed as a floating point value (degrees)
    :param bbox_arr: A 2x2 array expressed in (lon, lat) pairs
    :param range_max: Maximum range expressed in terms of geodesic curve distance.  Defaults to units of meters
    """
    ...
    flag_arr[(lon < bbox[0][0]) | (lat < bbox[0][1]) |
             ...
             (np.isnan(lon)) | (np.isnan(lat))] = QCFlags.FAIL
    return flag_arr
...
def range_check(arr, sensor_span, user_span=None):
    ...
    range as bad data.  Optionally, flag data which falls outside of a user
    defined range as suspect.
    ...
    :param user_span: An optional 2-tuple consisting of the expected data bounds. Data outside of these bounds but
                      within sensor span will be flagged as suspect.  If the parameter is not present or is None, then
                      only the sensor_span will be considered and no data can be flagged as suspect.
    """
    ...
    flag_arr[(arr < s_span_sorted[0]) |
             (arr > s_span_sorted[1])] = QCFlags.FAIL
    return flag_arr
...
def _process_time_chunk(value_pairs):
    ...
    or not, or returns NOT_EVALUATED if the threshold is None.
    """
    vals = value_pairs[0]
    ...
    if threshold is not None:
        ...
        return pd.Series(np.repeat(QCFlags.NOT_EVALUATED, len(vals)), vals.index,
                         dtype='i4')
...
def spike_check(times, arr, low_thresh, high_thresh, prev_qc=None):
    ...
    quantity, seeing if it exceeds a low or high threshold.
    ...
    :param low_thresh: The low threshold value
    :param high_thresh: The high threshold value
    :param prev_qc: An array of any previous QC values which were applied.  The first element is assumed to correspond
                    to the position of the first element of arr
    """
    ...
    idx1 = np.where((np.diff(times) > dt) & (flag_arr[1:] == QCFlags.FAIL))[0]
...
def rate_of_change_check(times, arr, thresh_val, prev_qc=None):
    ...
    :param thresh_val: Either a float value representing a rate of change over time or a quantities object representing
                       a value's rate of change over time
    :param prev_qc: An array of any previous QC values which were applied.  The first element is assumed to correspond
                    to the position of the first element of arr
    """
    ...
    # we must have some elements to apply the previous qc to, otherwise
    # don't do anything
    if arr.size > 0:
        if prev_qc is not None:
            flag_arr[0] = prev_qc[0]
        else:
            flag_arr[0] = QCFlags.NOT_EVALUATED
...
def flat_line_check(arr, low_reps, high_reps, eps, prev_qc=None):
    ...
    :param eps: a floating point number indicated the bounds within which the absolute
                difference of the previous and current measurement will be considered a repeated value.
    :param prev_qc: An array of any previous QC values which were applied.  The first element is assumed to correspond
                    to the position of the first element of arr
    """
    if not eps:
...
def time_series_flat_line_check(arr, low_reps=3, high_reps=5, eps=0.001, prev_qc=None):
    """
    Check for invariate observations that can be applied to all bulk wave
    parameters.
    :param arr: An array of observed data
    :param low_reps: number of repetitions prior to data being flagged suspect.
    :param high_reps: number of repetitions prior to being flagged bad.
    :param eps: a floating point number indicating the bounds within which the absolute
                difference of the previous and current measurement will be considered a repeated value.
    :param prev_qc: An array of any previous QC values which were applied.  The first element is assumed to correspond
                    to the position of the first element of arr
    """
...
def attenuated_signal_check(arr, times, min_var_warn, min_var_fail,
    ...
    :param times: An array of timestamps corresponding to the observed values
    ...
    flag_arr.fill(QCFlags.NOT_EVALUATED)
    ...
    if check_val < min_var_fail:
        flag_arr[time_idx] = QCFlags.FAIL
    elif check_val < min_var_warn:
        flag_arr[time_idx] = QCFlags.SUSPECT
    else:
        flag_arr[time_idx] = QCFlags.PASS
    return flag_arr
...
def qc_compare(vectors):
    ...
    priorities = [QCFlags.MISSING, QCFlags.NOT_EVALUATED, QCFlags.PASS,
                  QCFlags.SUSPECT, QCFlags.FAIL]
